    ...
    <div className="flex flex-wrap justify-between items-center gap-2 grow border-t border-dashed border-gray-200 px-4 sm:px-6 py-4 !z-40 bg-white shadow-sm rounded-b-xl">
      <div className="flex items-center gap-2">
        <Iconify
          ...
          className={` cursor-pointer text-primary   ${currentPage === 1
        ...
        {totalPages === 0 ? (
          ...
          generatePageNumbers?.map((pageNumber) => (
            <button
              key={pageNumber}
              onClick={(e) => { handlePageChange(pageNumber), e.preventDefault() }}
              className={`rounded-lg cursor-pointer w-8 h-8 outline-none text-primary   ${pageNumber === currentPage
        ...
        <Iconify
          ...
          className={`cursor-pointer text-primary ${currentPage === totalPages
            ? "cursor-not-allowed opacity-40 pointer-events-none"
            : ""
            }`}
        />
          ...
          <div className="absolute bottom-10 left-0 flex flex-col w-full bg-white shadow-md rounded-md !z-40 overflow-hidden">
            {rowValues?.map((item) => (
              <span
                ...
                className={`flex text-black items-center w-full h-8 px-2 hover:bg-tertiary ${rowsPerPage === item.value
