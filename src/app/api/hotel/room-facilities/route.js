...
export async function POST(req) {
  try {
    ...
    const prevFacilites = await prisma.room_facilities.findMany({
      where: {
        user_id: user_id,
      },
    });
    ...
    const updatedRoomFacilites = await prisma.room_facilities?.findMany({
      where: {
        user_id: user_id,
      },
    });
    ...
    return NextResponse.json(
      { message: "Success", facilites: updatedRoomFacilites },
      { status: 201 }
    );
  } catch (error) {
